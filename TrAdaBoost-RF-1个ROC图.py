...
利用全区因子栅格和分区栅格，对每个分区训练TrAdaBoost-RF模型并预测易发性
"""
...
factor_names = ['DEM_1km_WGS84', 'Slope', 'Aspect', 'plan_curv', 'profile_curv', 'Curvatu', 'annual_mean_T',
                'FVC_1km_WGS84', 'RAIN_1km_WGS84']
...
scaler_path = r"D:\arcgis_practice\Model\RF_rain4\scaler.pkl"
...
# --- 输出根目录 ---
output_root = r"D:\arcgis_practice\Loess_Plateau\TrAdaBoost_Results_ByRF0701"
os.makedirs(output_root, exist_ok=True)

# --- TrAdaBoost 超参数 ---
N_ITER = 20  # 迭代次数
RF_N_EST = 100  # 每个基随机森林的树数量
RF_MAX_DEPTH = 10
RF_MIN_SPLIT = 5
...
class TrAdaBoostRF:
    def __init__(self, base_rf_params=None, n_estimators=20):
        self.n_estimators = n_estimators
        self.base_rf_params = base_rf_params if base_rf_params else {
            'n_estimators': 100,
            'max_depth': 10,
            'min_samples_split': 5,
    ...
    def fit(self, X_source, y_source, X_target, y_target):
        ...
        for t in range(self.n_estimators):
            rf = RandomForestClassifier(**self.base_rf_params)
            rf.fit(X_all, y_all, sample_weight=weights)
            self.models.append(rf)

            # 预测
            y_pred_source = rf.predict(X_source)
            y_pred_target = rf.predict(X_target)
...
def main():
    ...
    for zone_id in unique_zones:
        ...
        tr_model = TrAdaBoostRF(base_rf_params=rf_params, n_estimators=N_ITER)
